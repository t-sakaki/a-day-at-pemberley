...
(built by build_interiors.py / build_grand_hall.py / build_upstairs.py),
drops the same ceiling/chandelier objects build_walk_views.py cuts away so a
camera inside the room stays unobstructed, and writes public/blender/walk/
<room>.glb. Room-local coordinates already match InteriorNavigation.ts (no
axis-swap transform here, unlike build_estate.py): x is room-local x, y is
room-local y (becomes glTF's -Z), z is up (becomes glTF's Y).
"""
import bpy, sys
