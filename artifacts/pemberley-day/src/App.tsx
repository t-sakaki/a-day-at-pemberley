...
const GUEST_PORTRAIT_KIND: Record<string, PortraitKind> = {
  'the-gardiners': 'gent',
  'elizabeth-bennet': 'lady',
  darcy: 'gent',
  georgiana: 'lady',
  bingley: 'gent',
  caroline: 'lady',
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (currentProps.mode === 'game') {
        ...
        // 芝生を歩く来客（ダーシー氏の帰館以降の再会場面など）。
        (currentProps.visitors ?? []).forEach((visitor, vi) => {
          const spot = ROAMING_VISITORS[visitor.id];
          if (!spot) return;
          const key = `visitor-${visitor.id}`;
          const current = staffMotionRef.current[key] ?? { ...spot.home };
          const roam = {
            x: spot.home.x + Math.sin(t * (0.16 + vi * 0.02) + vi * 2) * 1.6,
            y: spot.home.y + Math.cos(t * (0.13 + vi * 0.02) + vi * 2) * 1.0,
          };
...
function DiaryModal({ entry, language, taskCount, onReset, onClose, triggerRef, entries, onSelectEntry }: { entry: DiaryEntry; language: Language; taskCount: number; onReset: () => void; onClose: () => void; triggerRef: RefObject<HTMLElement | null>; entries?: DiaryEntry[]; onSelectEntry?: (entry: DiaryEntry) => void }) {
  ...
  const { isPro } = usePemberleyPro();
  // Free households keep the three most recent diary entries; Pemberley Pro keeps them all.
  const visibleEntries = entries && (isPro ? entries : entries.slice(-3));
...
function App() {
  ...
  const estateVisitors = useMemo<Visitor[]>(() => guestStates
    .filter(guest => ROAMING_VISITORS[guest.id])
    .map(guest => ({
      id: guest.id,
      kind: ROAMING_VISITORS[guest.id].kind,
      color: guest.color,
      label: language === 'ja' ? guest.nameJa : guest.name,
      expression: guest.id === 'darcy' && guest.mood < 80 ? 'tense' : moodExpression(guest.mood),
    })), [guestStates, language]);
  ...
  useEffect(() => {
    ...
    events.forEach(event => {
      if (minutes >= event.at && !firedEventsRef.current.has(event.id) && event.shouldFire !== false) {
        ...
        announce(event.message, event.tone, event.id);
      }
    });
  ...
  useEffect(() => {
    ...
    scheduledEmergencies.forEach(({ type, at }) => {
      if (minutes >= at && !emergencySpawnedRef.current.has(type)) {
        ...
        if (event) announce(event.dialogue, type === 'sick' ? 'warning' : 'arrival', `emergency-${type}`);
      }
    });
    if (minutes >= 13 * 60) {
    ...
    if (escalated) {
      ...
      announce(escalated.escalationDialogue, 'warning', `emergency-${escalated.type}-escalated`);
      const followOn = active.find(event => event.parentId === escalated.id);
      if (followOn) announce(followOn.dialogue, 'warning', `emergency-${followOn.type}`);
    }
  ...
  const answerLetter = useCallback((letterId: string, optionIndex: 0 | 1) => {
    ...
    audioRef.current?.eventTone('report');
  }, [addLog, notify, language, letterChoices]);
  ...
  const startGame = () => {
    ...
    voiceRef.current?.play('day-start', language);
  };
  const takeWalk = () => {
    ...
    <div className="pemberley-app game-shell">
      ...
      {notice && <div className="toast-note">{notice}</div>}
        <Dialog open={settingsOpen} triggerRef={settingsTriggerRef} onClose={() => setSettingsOpen(false)} label={t('settings')}>
          <button className="icon-button" style={{ float: 'right', color: '#31554c' }} onClick={() => setSettingsOpen(false)} aria-label={t('close')}><X size={17} /></button><div className="eyebrow" style={{ color: '#a36b48' }}>{t('correspondence')}</div><h2>{t('settings')}</h2><p>{language === 'ja' ? '一日の音や表示を整えます。進行状況はこのブラウザに保存されます。' : 'Adjust the sensory details of your day. Your progress is kept in this browser.'}</p><div style={{ marginTop: 24, borderTop: '1px solid #cdbc9e' }}><label style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', borderBottom: '1px solid #cdbc9e', fontSize: 12 }}>{t('language')} <select value={language} onChange={event => setLanguage(event.target.value as Language)}>{languages.map(item => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><label style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', borderBottom: '1px solid #cdbc9e', fontSize: 12 }}>{t('sound')} <input type="checkbox" checked={sound} onChange={event => setSound(event.target.checked)} /></label><label style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', borderBottom: '1px solid #cdbc9e', fontSize: 12 }}>{t('voice')} <input type="checkbox" checked={tts} onChange={event => setTts(event.target.checked)} /></label><label className="range-setting">{language === 'ja' ? '音声速度' : 'Voice speed'} <input type="range" min="0.5" max="2" step="0.1" value={voiceRate} onChange={event => setVoiceRate(Number(event.target.value))} /><output>{voiceRate.toFixed(1)}×</output></label></div>{pro.available && <div style={{ marginTop: 18, paddingTop: 14, borderTop: '1px solid #cdbc9e', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><div style={{ fontSize: 12 }}><strong>{language === 'ja' ? 'ペンバリー・プロ' : 'Pemberley Pro'}</strong><div style={{ color: '#61706a', fontSize: 10, marginTop: 2 }}>{pro.isPro ? (language === 'ja' ? 'ご利用中 · 日誌の全履歴' : 'Active · full diary history') : (language === 'ja' ? '領地を支援する' : 'Support the estate')}</div></div><button style={{ padding: '9px 13px', border: '1px solid #8b9b91', background: pro.isPro ? 'transparent' : '#31554c', color: pro.isPro ? '#31554c' : '#f2e8d2', fontSize: 11, cursor: 'pointer', flex: 'none' }} onClick={() => { setSettingsOpen(false); setProOpen(true); }}>{pro.isPro ? (language === 'ja' ? '管理' : 'Manage') : (language === 'ja' ? '支援する' : 'Unlock')}</button></div>}<div className="modal-actions"><button onClick={() => setSettingsOpen(false)}>{t('save')}</button></div>
        </Dialog>
        <PemberleyProDialog open={proOpen} onClose={() => setProOpen(false)} language={language} />
